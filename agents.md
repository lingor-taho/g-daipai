...
| 日期 | 问题 | 修复 |
|------|------|------|
| 2026-06-19 | 普通商品抓物流单号时，整页文本/HTML 中的隐藏 input 属性、id/value/class、以及 script 源码里的数字可能被误当作单号 | 插件普通商品待发货扫描的单号候选收窄为 `#messagelist` 消息区的渲染文字：优先读取 `innerText`，避免 `script/style/hidden input` 等非页面显示内容参与匹配；无消息区时保留旧 fallback。新增回归测试覆盖 `currentMsgId value="3095037709"` 和 script `hierarchyId: 2084614008` 早于真实单号时，仍只提取消息区可见文字中的 `390166447193`。验证：`node yahoo-plugin/content.test.js`、`node --check yahoo-plugin/content.js`、`node --check yahoo-plugin/content.test.js` |
