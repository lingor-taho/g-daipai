...
| 2026-06-09 | 商城购买确认页误点 PayPay `特典を確認する` 导致广告页反复打开 | 付款/购买流程的 `確認する` 现在优先限定在 `#confirm` 容器内；如 Yahoo 改动 id，则优先选择处于 `お支払い金額（税込）/支払い金額` 订单金额区域内的 `確認する`，避免匹配 PayPay 特典区域的同名子元素 |
...
- 2026-06-09 追加：购买确认页的目标 `確認する` 必须优先来自 `#confirm` 容器；如果 `#confirm` 不存在，再使用其前方同块内容含 `お支払い金額（税込）/支払い金額` 的订单金额上下文判断。不要只按全页 `確認する` 文案查找，否则可能误点 PayPay `特典を確認する` 内部子元素并反复打开广告页。
