...
修复后，结构化数据明确表示卖家承担运费时直接返回 `無料`；只要页面已有 `itemPostage` 容器或结构化运费责任/输入字段，就不再启用整页数字回退。确实需要兼容旧页面时，回退文本也会排除 `<head>` 和 `itemTitle`，防止标题、商品价进入运费候选。回归测试同时覆盖 seller 和 winner 两种运费责任、运费 DOM 只有骨架、标题附近当前价为 `52,000円` 的边界。
