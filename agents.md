...
| 2026-06-09 | 商城购买确认页误点 PayPay `特典を確認する` 导致广告页反复打开 | 付款/购买流程的 `確認する` 只从真正可点击元素中匹配完整文本严格等于 `確認する` 的按钮/链接，优先 `<a>確認する</a>`；广告链接完整文本是 `特典を確認する`，不再因内部子文字或相似文案被误点 |
...
- 2026-06-09 追加：购买确认页的目标 `確認する` 只从真正可点击元素中匹配完整文本严格等于 `確認する` 的按钮/链接，优先 `<a>確認する</a>`。不要把 `span` 子元素作为付款 `review` 候选，也不要用包含关系或相似文案判断；PayPay 广告链接完整文本为 `特典を確認する`，必须排除。
