...
| 日期 | 问题 | 修复 |
|------|------|------|
| 2026-06-22 | 后台“报表 / 真实输入明细”中个别记录商品 ID 显示为空。根因是插件 `trusted_input` 诊断日志从 URL/诊断文本反解析商品 ID 时，只识别字母开头的 Yahoo auction ID，遇到纯数字 ID（如 `1225922765`）会写入空 `product_id`。进一步检查主业务入口已基本支持纯数字 ID，仅 content 当前页面 ID 兜底仍只认字母开头 | `yahoo-plugin/background.js` 的 `extractAuctionIdFromText` 改为支持 `[a-zA-Z]?\d{8,10}`；`yahoo-plugin/content.js` 的 `getCurrentAuctionId` 兜底同步支持纯数字 ID，避免付款/物流页面把自身纯数字商品 ID 误当物流号。新增 background/content 测试覆盖纯数字 ID。验证：`node yahoo-plugin/content.test.js`、`node yahoo-plugin/background.test.js`、`node src/server/routes/task.test.js`、`node src/server/routes/plugin.test.js`、`node --check yahoo-plugin/content.js`、`node --check yahoo-plugin/background.js`、`node scripts/encoding-guard.js` |
| 2026-06-22 | 远程商品 `x1233517511` 出现旧任务 `失败：服务器tab异常`，现场表现为商品页刚打开就关闭、没有点击出价按钮。远程 debug 显示后续同商品任务已成功 `bidding/highest`，旧失败任务错误为 `Server tab error: No tab with id...`。进一步定位不是人工拖动/关闭 tab，而是并行调度下交易/扫描流程的 finally cleanup 会关闭流程开始后新开的所有 `auctions.yahoo.co.jp/*` tab，误把同时打开的出价商品页 `/jp/auction/...` 当作交易临时页关掉 | 收窄 `closeTabsForTransactionFlow` / `closeTabsForScanFlow` 的自动清理范围：只自动清理 `about:blank`、`contact.auctions.yahoo.co.jp`、`buy.auctions.yahoo.co.jp`、`login.yahoo.co.jp`、`account.edit.yahoo.co.jp` 这类交易/支付/验证页；不再按域名清理普通商品页 `/jp/auction/...`。流程自己记录的 created tab 仍会关闭。新增回归测试 `testTransactionCleanupDoesNotCloseNewAuctionProductTabs`，验证交易 cleanup 不会误关并行出价商品页。验证：`node yahoo-plugin/background.test.js` 中新增测试已通过，完整文件仍停在既有 `testBuyoutPendingFinalStaysBiddingForWonSync` 旧失败；另通过 `node --check yahoo-plugin/background.js`、`node --check yahoo-plugin/background.test.js`、`node scripts/encoding-guard.js`、`git diff --check` |
| 2026-06-22 | 远程商品 `s1233522728` 第一次付款失败、清除后第二次成功。远程 debug API 显示订单/商品数据正常，唯一诊断为 `payment button not found for trusted click`，发生在 `確認する` 点击后页面 5 秒未进入下一状态、插件尝试 Chrome debugger 真实鼠标兜底时短暂找不到按钮点位；这不是金额、运费或三表字段问题 | 付款 `review` 点击增加一次安全重试：synthetic click 超时后若 trusted click 因 `payment button not found for trusted click` 失败，先额外等待 3 秒确认是否已经跳页；若仍停留在付款页且 `確認する` 仍存在，等待 1 秒后重新执行一次 `review` 点击。第二次仍失败才按原逻辑上报失败。验证：新增 `testRunPaymentJobsRetriesReviewClickWhenTrustedPointTemporarilyMissing`；`node yahoo-plugin/background.test.js` 中该测试已通过，完整文件仍停在既有 `testBuyoutPendingFinalStaysBiddingForWonSync` 旧失败；另通过 `node --check yahoo-plugin/background.js`、`node --check yahoo-plugin/background.test.js`、`node scripts/encoding-guard.js`、`node src/client/src/utils/taskFailureReason.test.mjs` |
| 2026-06-22 | 远程任务偶发 `No tab with id` / `Tabs cannot be edited right now (user may be dragging a tab)`，以前直接永久失败并显示“系统原因”。这类错误来自服务器 Chrome tab 临时不可用、tab 被关闭/重载或用户正在拖动标签，任务通常还没真正进入出价动作 | 插件出价任务新增一次自动重试：首次命中临时服务器 tab 异常时关闭残留任务 tab，等待后重新打开商品页再执行；第二次仍失败才落库稳定错误 `Server tab error: ...`。前端/后台失败归类新增 `失败：服务器tab异常`。验证：`node --check yahoo-plugin/background.js`、`node --check yahoo-plugin/background.test.js`、`node src/client/src/utils/taskFailureReason.test.mjs`、`node scripts/encoding-guard.js`、`npm run build --prefix src/client`、`npm run build --prefix src/admin`。补充：完整 `node yahoo-plugin/background.test.js` 仍失败在既有 `testBuyoutPendingFinalStaysBiddingForWonSync`，新增 tab 重试测试位于该失败前并已执行通过 |
| 2026-06-22 | 远程商品 `d1233636509` 失败显示“系统原因”，但 debug API 原始错误是 `Auction ended according to product page snapshot`，任务提交时距离 Yahoo 结束只剩约 18 秒，插件处理到页面时拍卖已结束，没有进入实际出价 | 失败原因归类补充 `Auction ended according to product page snapshot`，前端显示改为 `失败：商品已结束`，避免误导为系统故障。验证：`node src/client/src/utils/taskFailureReason.test.mjs`、`node scripts/encoding-guard.js`、`npm run build --prefix src/client` |
| 2026-06-22 | 远程同捆商品 `1218905181` 第一次付款失败，但清除后重试成功。远程 debug API 显示失败不是金额或鑑定，而是 `payment shipping option 430円 not selectable`：同捆主单期望运费 `430円`，付款页当时已在同捆付款流程但没有可展开/可选择的配送方式按钮，插件在最终金额校验前先强制选择运费而失败 | 付款流程调整为：如果页面当前 `お支払い金額（合計）` 已等于系统应付金额，则不再强制寻找/选择配送方式；若尝试选择运费失败，也会重新读取页面状态，金额正确则继续进入 `確認する`。最终仍保留 `assertPaymentAmountMatches()` 做金额一致性校验。新增回归测试覆盖 `finalPrice=240`、同捆运费 `430円`、页面合计已是 `670円` 但没有可选配送项时应直接继续付款。验证：`node --check yahoo-plugin/background.js`、`node --check yahoo-plugin/background.test.js`、`node scripts/encoding-guard.js`、`git diff --check`。补充：完整 `node yahoo-plugin/background.test.js` 仍失败在既有 `testBuyoutPendingFinalStaysBiddingForWonSync`（`true !== false`），新增同捆运费测试位于该失败前并已执行通过 |
| 2026-06-22 | 远程商品 `g1233517461` 付款失败：`付款金额不一致（应付 22888円，页面 2500円）`。远程 debug API 显示订单和任务正常，插件诊断中 pageSample 混入广告/script 文本，且金额解析把 `鑑定` 说明里的 `鑑定する（2,500円）` 当成了付款页金额 | 付款金额解析改为优先识别真实合计标签 `お支払い金額（合計）/支払い金額（合計）`，并过滤 `お支払い金額が全額返金` 这类鑑定说明命中；付款页正文采集优先使用 `innerText`，减少 script/隐藏广告文本干扰。新增回归测试覆盖同页同时存在 `鑑定する（2,500円）` 和真实 `お支払い金額（合計）22,888円` 时必须解析为 `22888`。验证：`node --check yahoo-plugin/background.js`、`node --check yahoo-plugin/background.test.js`、`node scripts/encoding-guard.js`、`git diff --check`。补充：完整 `node yahoo-plugin/background.test.js` 仍失败在既有 `testBuyoutPendingFinalStaysBiddingForWonSync`（`true !== false`），新增金额解析测试位于该失败前并已执行通过 |
...
| 2026-06-21 | 三表删 `tasks` 商品冗余字段后的生产版本出现一系列问题：插件/API 临时失败会影响出价状态、后台统计 504、部分调用路径在删字段后仍依赖旧字段，导致生产风险不可接受；服务器已回档到删字段之前，后续不能继续基于删字段后的本地代码推进 | 当前基线回到提交 `1bc4b125f8130103b1d08b61d4ae2d9b5d8370af`（`商城即决`，2026-06-21 11:00 +0800）。数据库也回到删字段前结构，`tasks` 继续保留 `product_url/product_title/product_image_url/current_price/end_time/buyout_price/tax_type/shipping_fee_text/product_type/bid_count` 等商品快照字段。三表计划调整为：先在保留旧字段的稳定基线上重新完成“单写、读取收口、诊断和回归覆盖”，确认所有运行路径不再依赖旧字段后，再单独执行删字段迁移；删字段脚本和生产执行步骤必须重新验证，不得复用之前出问题的删字段后代码状态。本地已验证：`node scripts/encoding-guard.js`、`node scripts/check-product-health.js`、`node scripts/check-product-read-paths.js` 均通过 |
| 2026-06-21 | 商城即决商品未勾选“まとめて購入”的场景，Yahoo 实际已经拍下，但插件可能先把任务标为失败，后续 `/my/won` 落札同步再把同商品失败任务改为成功。生产商品 `x1234327757` 远程 debug API 证据：任务最终 `success`、订单 `final_price=14960` 已存在，但 `bid_count=0`、`last_bid_at=null`、`bidLogs=[]`，说明插件未走到正常 bidding 回写，成功来自落札同步兜底 | 根因是商城即决最后点击 `購入を確定する` 后，Yahoo 订单页/thank-you 跳转可能超过插件 pending final 重试窗口；旧逻辑会把 `buyout-final-waiting` 最终当 `final bid confirmation pending timeout` 写成 failed。`background.js` 保持普通/即决任务外层 30 秒超时不变，避免影响出价队列吞吐；仅当 content 已明确返回 `buyout-final-waiting`（表示最终购买按钮已提交）时，不再继续 3 次重试或写 failed，而是回写 `bidding` 并关闭任务页，等待落札同步确认最终 `success`。验证：`node yahoo-plugin/background.test.js`、`node yahoo-plugin/content.test.js`、`node --check yahoo-plugin/background.js`、`node --check yahoo-plugin/background.test.js` |
...
| 2026-06-20 | 用户端“入札中”页面需要显示 Yahoo 入札中列表红框处的剩余时间，例如 `5分/1時間` | 插件 `/my/bidding` 同步新增从当前商品行内 `svg[aria-label="残り時間"]` 所在元素读取 `remainingTimeText`；`bidding_items` 新增 `remaining_time_text` 字段并由 `/api/plugin/bidding/sync` 写入；用户端 `/api/task/bidding` 返回该字段，`ActiveBidding` 在“最近更新时间”上方用红色小时钟图标显示“剩余时间”。验证：`node yahoo-plugin/content.test.js`、`node src/server/routes/plugin.test.js`、`node src/server/routes/task.test.js`、`node --check yahoo-plugin/content.js`、`node --check src/server/routes/plugin.js`、`node --check src/server/routes/task.js`、`node --check src/server/models/index.js`、`npm run build --prefix src/client` |
| 2026-06-20 | `start.bat` 只在启动 20 秒后检查一次 3034 端口，API 子进程后续退出时外层服务窗口仍显示“Services are running”，导致后台和插件访问 `localhost:3034` 失败但不易发现 | 新增 `scripts/api-watch.bat`，由 `start.bat` 启动 API watcher 循环运行 `node src/server/index.js`；API 退出时写入 `server-start.err.log` 退出时间和退出码，5 秒后自动重启。`start.bat` 启动前会清理旧 watcher 和 3034/3035/8000 监听进程，并重建 `server-start.log/server-start.err.log`，避免 API 静默掉线。生产更新后需把 `start.bat` 和 `scripts/api-watch.bat` 同步到 `C:\www\g-daipai`。验证：`cmd /c scripts\api-watch.bat` 未完整运行（会进入常驻循环），已做批处理结构检查。 |
| 2026-06-20 | 项目隔一段时间出现 UTF-8/GBK/ANSI 乱码，可能把原本正常的源码、注释、配置或 Google 工作表页签名改坏；原防线只覆盖 `yahoo-plugin/content.js` 和 `background.js`，服务端、后台、配置和测试注释仍可能漏检 | 新增全项目编码闸门 `scripts/encoding-guard.js` 和测试 `scripts/encoding-guard.test.js`，扫描 Git 管理文本文件与本地 `.env`，严格 UTF-8 解码，拦截替换字符、常见 mojibake 片段和旧 `-代拍表-` 默认值，并跳过 `.git/node_modules/data/logs/.umi` 等非维护内容；`npm run encoding:guard` 可单独运行，`npm run regression` 首步自动执行。清理当前源码中残留的可见乱码：删除后台 `Tasks.tsx` 未使用重复失败归类函数，将 `taskFailureReason.js` 和 `googleSheets.js` 的旧乱码兼容匹配改为 Unicode escape，修复 `plugin.js/plugin.test.js` 乱码注释。本地检查：源码/配置无 UTF-8 解码失败；`.env` 未命中可疑 Google/sheet/乱码配置；SQLite `config` 表当前无 Google sheet 配置行，运行默认 `-Ygao-`。验证：`node scripts/encoding-guard.test.js`、`npm run encoding:guard`、`node src/client/src/utils/taskFailureReason.test.mjs`、`node src/server/services/googleSheets.test.js`、`node src/server/routes/plugin.test.js`、`npm run build --prefix src/admin`、`npm run regression` |
| 2026-06-20 | `scripts/check-product-read-paths.js` 报 `admin.js` 中 `task_*` 调试字段和即决最高价显示字段违规，但 `check-product-parity.js` 为 0，三表数据实际一致 | 修正只读路径扫描脚本：允许调试接口显式返回 `t.xxx AS task_xxx` 与 `p.xxx AS product_xxx` 的三表对比字段；允许即决任务最高价显示表达式 `COALESCE(t.user_max_price, t.buyout_price, t.max_price)`，该处 `buyout_price` 是任务出价/购买金额展示，不是商品快照读路径。验证：`node scripts/check-product-read-paths.test.js`、`node scripts/check-product-read-paths.js`、`node scripts/check-product-parity.js`、`node --check scripts/check-product-read-paths.js`、`node --check scripts/check-product-read-paths.test.js` |
| 2026-06-20 | 服务器 Google 工作表页签名显示乱码，导致写入 Google 表格找不到工作表；中文默认页签名在服务器配置/环境变量中有编码风险 | Google 工作表默认页签名从 `-代拍表-` 改为英文 `-Ygao-`；后端对明显乱码的 sheet name（替换字符或常见 mojibake 片段）自动忽略并回退默认值；后台系统配置和“待收货补表格”默认文案同步使用 `-Ygao-`。服务器更新后如 `config.google_sheets_sheet_name` 仍为乱码，运行新代码会自动使用 `-Ygao-`，也可在后台勾选“允许修改 Google 表格配置”后保存为 `-Ygao-`。验证：`node src/server/services/googleSheets.test.js`、`node --check src/server/services/googleSheets.js`、`node --check src/server/services/googleSheets.test.js`、`npm run build --prefix src/admin` |
| 2026-06-19 | 店铺即决/购买手续商品提交后，后台“最高价”可能比 Yahoo 页面显示价低 1 日元，例如税前 `2856` 页面含税显示 `3,142円`，旧逻辑用 `Math.floor(2856 * 1.1)` 得到 `3,141`；同时即决任务保存时又把含税即决价折回税前价，导致 `u1051658399/t1110559761` 这类商品显示最高价低于当前价并可能提交失败；商城即决页勾选 `この出品者の他の商品とまとめて購入する` 后必须等待并点击 `今すぐ落札する`，不能走其他确认按钮 | 店铺即决 `pageData.items.winPrice` 转页面显示即决价时改为 `Math.round(value * 1.1)`，匹配 Yahoo 页面含税显示；即决任务保存 `max_price/user_max_price/buyout_price` 统一保留页面显示即决价，不再对 `tax_included` 即决价折税前；后台任务列表和队列统计对历史即决任务优先显示 `user_max_price/buyout_price`；插件即决执行路径明确跳过价格输入框，只点击 `購入手続きへ/今すぐ落札` 等按钮；商城即决勾选 `まとめて購入` 后最多等待 10 秒 `今すぐ落札する`，出现后只点击该按钮，未出现则报明确错误。注意：`tasks.max_price` 从此不能被假设为所有场景税前价，普通出价仍是 Yahoo 出价口径，`bid_mode=buyout` 是页面购买金额口径；订单结算仍只应使用 `orders.final_price`，该值来自 Yahoo 落札/订单同步，不应从 `tasks.max_price` 兜底。真实抓取验证 `q1222339778=1,047円`、`o1113090605=523円`。验证：`node src/shared/biddingRules.test.cjs`、`node src/server/routes/task.test.js`、`node src/server/routes/proxy.test.js`、`node yahoo-plugin/content.test.js`、`node src/server/routes/admin.orders.test.js`、`node --check src/shared/biddingRules.cjs`、`node --check src/server/routes/proxy.js`、`node --check yahoo-plugin/content.js`、`node --check src/server/routes/admin.js` |
...
| 2026-06-15 | 对比历史提交确认乱码不是 Yahoo 页面问题，而是源码文件曾被 UTF-8 按 GBK/ANSI 解码后再次保存，导致日文/中文注释和少量运行字符串变成常见 GBK/UTF-8 互转乱码字符 | 新增 `.editorconfig` 和 `.gitattributes`，统一文本文件 `UTF-8 + LF`；清理 `content.js` 中已写入的乱码注释，恢复 `cleanupProductTitle()` 的 `商品 {auctionId}` fallback 为 `\u5546\u54c1` escape；`encoding.test.js` 增加常见 mojibake 字符检测，插件文件再次出现 BOM、替换符、GBK/ANSI 乱码或复制截断标记时测试直接失败。验证：`node yahoo-plugin/encoding.test.js`、`node yahoo-plugin/content.test.js`、`node yahoo-plugin/background.test.js`、`node --check yahoo-plugin/content.js`、`git diff --check` |
| 2026-06-15 | 对比 `4ebdc8e` 发现当时落札价还能抓到，后续版本抓不到的直接差异在 `yahoo-plugin/content.js`：`extractOrderHistory()` 内查找纯价格 DOM 叶子节点的正则从 `数字+円` 被转码损坏成 `数字+乱码字符`，导致部分 Yahoo 落札记录的独立价格节点无法解析 | 将纯价格节点匹配改为稳定 ASCII 写法 `(?:\u5186|JPY)`，移除损坏的乱码兜底，避免误抓或漏抓；新增真实 `円` 叶子价格测试，覆盖标题编号和落札价粘连时仍优先取独立价格节点。验证：`node yahoo-plugin/content.test.js`、`node yahoo-plugin/encoding.test.js`、`node --check yahoo-plugin/content.js` |
...
| 2026-06-09 | 插件 JS 中存在不可恢复乱码字符 | 清理 `background.js` 中已坏掉的替换字符/GBK 乱码字符串，改为英文稳定错误/日志和 `\u5186`；新增 `yahoo-plugin/encoding.test.js`，扫描 `content.js/background.js`，发现替换字符或 GBK 乱码立即失败，避免后续继续混入不可恢复乱码 |
...
| 2026-06-11 | 系统配置中初始化订单状态入口风险较高，Google 表格页签名不可配置 | 后台“系统配置 / 交易开始任务”彻底取消“初始化订单状态”功能：前端按钮和后端 `/api/admin/transaction-start/reset-orders` 接口均移除。Google 表格配置新增“Google工作表名称”，保存到 `config.google_sheets_sheet_name`，后端追加/查找 Google 表格时优先使用该配置，未配置时继续兼容 `GOOGLE_SHEETS_SHEET_NAME` 和当时默认 `-代拍表-`。当前默认已改为 `-Ygao-`，旧 `-代拍表-` 仅为历史记录 |
| 2026-06-11 | 后续开发计划重新确认 | 生产环境暂未发现 `/my/won` 落札同步、默认信用卡付款、交易 tab 关闭问题；出价/查询 worker 隔离和批处理危险操作收口先保留以后讨论。PIN/验证码仍待继续处理：当前进入验证码页面后没有把验证码图片返回后台，并且还会再次打开 PIN 页面。待收货补表格页面说明不再写死当时默认的 `-代拍表-`，改为读取当前 Google 工作表名称配置。当前默认已改为 `-Ygao-`，旧 `-代拍表-` 仅为历史记录 |
...
- 操作页签：当时默认 `-代拍表-`。当前默认已改为 `-Ygao-`，旧 `-代拍表-` 仅为历史记录。
- 追加字段顺序：
  - `落札日期`
  - `用户名`
  - `商品链接`
...
- `src/server/services/googleSheets.js` 新增 Google Sheets API 客户端：
  ...
  - 当时默认 sheet name：`-代拍表-`；当前默认已改为 `-Ygao-`，旧 `-代拍表-` 仅为历史记录
  - 支持环境变量：
...
- 批处理复用扫描发货后的同一套 Google 表格追加逻辑：
  - 当时写入 `-代拍表-` 页；当前默认已改为 `-Ygao-`，旧 `-代拍表-` 仅为历史记录。
