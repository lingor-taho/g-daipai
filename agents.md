...
| 2026-06-09 | 商城购买确认页误点 PayPay `特典を確認する` 导致广告页反复打开 | 付款/购买流程的 `確認する` 必须位于 `支払い金額 / お支払い金額（税込）` 订单金额区块或 `#confirm` 容器内；找不到订单金额上下文时直接失败，不再回退点击全页其它 `確認する`，避免匹配 PayPay 特典区域的同名子元素 |
...
- 2026-06-09 追加：购买确认页的目标 `確認する` 必须来自其前方同块内容含 `支払い金額 / お支払い金額（税込）` 的订单金额上下文，或明确处于 `#confirm` 容器内。找不到该订单区域时直接失败，不再按全页 `確認する` 文案兜底，否则可能误点 PayPay `特典を確認する` 内部子元素并反复打开广告页。
